    ...
    setTimeout(async () => {
        ...
        try {
            ...
            for (const inv of freshInvites) {
                ...
                if (cached && currentUses > cached.uses) {
                    inviterId = cached.creatorId;
                }
        ...
        try {
            ...
            for (const roleName of RANK_ORDER) {
                ...
                if (roleObj && inviterRoles.includes(roleObj.id)) {
                    topRole = roleName;
                    break; 
                }
            }
            ...
            if (topRole === "Manager") {
                ...
                if (!staffChannel) {
                    staffChannel = await server.createChannel({ name: "bot-roles", type: "TextChannel" });
                }

                await staffChannel.sendMessage(`<@${inviterId}> invited <@${memberId}>! Reply with: !role <@${memberId}> [Role]`);
                
                if (PENDING_MGR.has(inviterId)) clearTimeout(PENDING_MGR.get(inviterId).timeout);
                PENDING_MGR.set(inviterId, { 
                    target: memberId, 
                    timeout: setTimeout(() => PENDING_MGR.delete(inviterId), 3600000) 
                });
            
            } else if (topRole) {
                const index = RANK_ORDER.indexOf(topRole);
                if (index >= 0 && index < RANK_ORDER.length - 1) {
                    await assign(member, RANK_ORDER[index + 1]);
                }
            }
        } catch (err) {}
    }, 1500); 
...
async function assign(member, roleName) {
    ...
    if (!currentRoles.includes(role.id)) {
        const rankRoleIds = RANK_ORDER.map(rName => getRoleByName(member.server, rName)).filter(r => r).map(r => r.id);
...
client.on("serverDeleted", (serverId) => {
    for (const [id, data] of INVITE_CACHE) {
        if (data.serverId === serverId) INVITE_CACHE.delete(id);
    }
});
